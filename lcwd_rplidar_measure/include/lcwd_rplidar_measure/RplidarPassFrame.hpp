...
class RplidarPassFrame
{
    ...
    private:
	    ...
	    vector<Pole_position> vector_pole_left_,vector_pole_right_;	
        ...
        float dist_left_, dist_right_, angle_left_, angle_right_, speed_range_, speed_angle_;
        ...
        void rplidar_pole_distance(vector<float> vec_a,float &dist_left,float &angle_left,float &dist_right,float &angle_right,float minAngle,float increment);
        ...
        double compute_angle_mean(const vector<Pole_position> &pole_vec);
        double compute_dist_mean(const vector<Pole_position> &pole_vec);
        double compute_angle_var(const vector<Pole_position> &pole_vec);
        float cos_theorem(float a,float b,float c);
